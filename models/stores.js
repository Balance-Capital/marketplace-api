...
const getStoreByName = async function getStoreByName(name) {
  const Name = new RegExp(`.*${name?.replace('.','\\.')}.*`,'gui');
  try {
    const store = await this.find({ name: Name }).exec();
    return store[0];
  } catch (error) {
    logger.error(error);
    return null;
...
Stores.statics.getStoreByName = getStoreByName;
